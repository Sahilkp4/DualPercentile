"""
Stage 3 - Threshold Discovery Pipeline (Main Orchestrator) - OPTIMIZED VERSION
...
- Multi-core parallel processing at strategy level (10-16x speedup)
- Per-strategy random seeding for reproducibility
- Thread-safe logging (console-only in parallel mode)

Pipeline Flow:
1. Load configuration and data
2. For each Stage 2 strategy (in parallel):
...
# Import Stage 3 modules (use optimized versions where available)
...
def setup_logging(config: dict) -> logging.Logger:
    """
    Setup logging for Stage 3 (thread-safe for parallel processing).
    ...
    # File handler - only add if not using multiprocessing (avoid conflicts)
...
def process_single_strategy(
    ...
    ----------
    strategy_idx : int
        Strategy index (for logging and seeding)
    strategy : pd.Series
    ...
    """
    # Set per-strategy random seed for reproducibility in parallel execution
    strategy_seed = config['bootstrap']['random_seed'] + strategy_idx
...
def main():
    ...
    # Parallel or sequential processing based on config
    if config['performance']['use_multiprocessing']:
        n_jobs = config['performance']['n_jobs']
        logger.info(f"Using multiprocessing with n_jobs={n_jobs}")
        logger.info("Note: File logging disabled in parallel mode (console only)")

        # Create list of strategy tuples for parallel processing
        strategies_list = [
        ...
        all_results = Parallel(n_jobs=n_jobs, verbose=10)(
            delayed(process_single_strategy)(*args)
            for args in strategies_list
        )
    else:
        logger.info("Multiprocessing disabled, using sequential processing")
        for idx, (_, strategy) in enumerate(stage2_strategies.iterrows()):
